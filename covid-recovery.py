...
# Change text to anything you wish, but must be 16 characters or less
  akRecovery  = alaskaRecovered(requests.get("https://covidtracking.com/api/v1/states/current.json").json())
  worldRecovery = worldRecovered(requests.get("https://api.covid19api.com/summary").json())
  
  akRecovery = ('{:,}'.format(akRecovery))
  worldRecovery = ('{:,}'.format(worldRecovery))
  while True:
    
    lcd_text("    COVID-19",LCD_LINE_1)
    lcd_text(" Recovery  Data",LCD_LINE_2)
    
    time.sleep(2)
    
    lcd_text("Alaska:", LCD_LINE_1)
    lcd_text(akRecovery, LCD_LINE_2)
    
    time.sleep(4)
    
    lcd_text("World:", LCD_LINE_1)
    lcd_text(worldRecovery, LCD_LINE_2)
